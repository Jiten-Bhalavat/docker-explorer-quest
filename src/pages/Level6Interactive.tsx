...
const AnimImageCmds = ({ onDone }: { onDone: () => void }) => {
  ...
  useEffect(() => {
    const timers = [
      setTimeout(() => {
        ...
        setTimeout(() => setTermLines(p => [...p, { text: 'alpine: Pulling from library/nginx' }]), 200);
        setTimeout(() => setTermLines(p => [...p, { text: '✓ sha256:a3... Pull complete', isSuccess: true }]), 400);
        setTimeout(() => setTermLines(p => [...p, { text: '✓ sha256:b7... Pull complete', isSuccess: true }]), 550);
        setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => setTermLines(p => [...p, { text: '18-slim: Pulling from library/node' }]), 200);
        setTimeout(() => setTermLines(p => [...p, { text: '✓ 3 layers pulled', isSuccess: true }]), 400);
        setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => setTermLines(p => [...p, { text: 'REPOSITORY    TAG        SIZE' }]), 150);
        setTimeout(() => setTermLines(p => [...p, { text: 'nginx         alpine     23.4MB' }]), 300);
        setTimeout(() => setTermLines(p => [...p, { text: 'node          18-slim    174MB' }]), 450);
        setTimeout(() => setImages(prev => prev.map(i => ({ ...i, isNew: false, highlighted: true }))), 500);
        setTimeout(() => setImages(prev => prev.map(i => ({ ...i, highlighted: false }))), 1200);
      }, 2300),

      setTimeout(() => {
        setStep(4);
        setTermLines(p => [...p, { text: "docker image inspect nginx:alpine --format '{{.Os}}/{{.Architecture}}'", isCmd: true }]);
        setTimeout(() => setTermLines(p => [...p, { text: 'linux/amd64' }]), 200);
        setTimeout(() => setImages(prev => prev.map(i => i.name === 'nginx' ? { ...i, infoBadge: 'linux/amd64' } : i)), 250);
        setTimeout(() => setImages(prev => prev.map(i => ({ ...i, infoBadge: undefined }))), 1100);
      }, 3000),

      setTimeout(() => {
        setStep(5);
        setTermLines(p => [...p, { text: 'docker rmi node:18-slim', isCmd: true }]);
        setTimeout(() => setTermLines(p => [...p, { text: 'Untagged: node:18-slim' }]), 150);
        setTimeout(() => setTermLines(p => [...p, { text: 'Deleted: sha256:b7c3...' }]), 300);
        setTimeout(() => setTermLines(p => [...p, { text: '✓ Removed', isSuccess: true }]), 450);
        setTimeout(() => setImages(prev => prev.filter(i => i.name !== 'node')), 400);
      }, 3600),
...
const AnimContainerCmds = ({ onDone }: { onDone: () => void }) => {
  const [termLines, setTermLines] = useState<{ text: string; isCmd?: boolean; isSuccess?: boolean }[]>([]);
  ...
  useEffect(() => {
    const timers = [
      ...
      setTimeout(() => {
        ...
        setTimeout(() => setTermLines(p => [...p, { text: 'CONTAINER ID  IMAGE          STATUS       PORTS              NAMES' }]), 150);
        setTimeout(() => setTermLines(p => [...p, { text: 'a1b2c3d4      nginx:alpine   Up 5 sec     0.0.0.0:8080->80   web' }]), 300);
        setTimeout(() => setTermLines(p => [...p, { text: 'e5f6g7h8      postgres:15    Up 3 sec     0.0.0.0:5432->5432 db' }]), 450);
      }, 2100),
      ...
      setTimeout(() => {
        ...
        setTimeout(() => setTermLines(p => [...p, { text: 'docker ps -a', isCmd: true }]), 500);
        setTimeout(() => setTermLines(p => [...p, { text: 'e5f6g7h8  postgres:15  Up 12 sec  db' }]), 650);
      }, 3700),
...
const AnimExecLogs = ({ onDone }: { onDone: () => void }) => {
  const [termLines, setTermLines] = useState<{ text: string; isCmd?: boolean; isSuccess?: boolean }[]>([]);
  ...
  useEffect(() => {
    const timers = [
      ...
      setTimeout(() => {
        ...
        setTimeout(() => setTermLines(p => [...p, { text: '/ # ls' }]), 200);
        setTimeout(() => setTermLines(p => [...p, { text: 'bin  dev  etc  home  lib  node_modules  proc  usr' }]), 400);
        setTimeout(() => setTermLines(p => [...p, { text: '/ # cat /etc/os-release' }]), 600);
        setTimeout(() => setTermLines(p => [...p, { text: 'NAME="Alpine Linux"' }]), 750);
        setTimeout(() => setTermLines(p => [...p, { text: 'ID=alpine' }]), 850);
        setTimeout(() => { setView('fs'); setBadge('EXEC SESSION'); }, 300);
      }, 1100),

      setTimeout(() => {
        setStep(3);
        setTermLines(p => [...p, { text: '/ # ps aux', isCmd: true }]);
        setTimeout(() => setTermLines(p => [...p, { text: 'PID  COMMAND' }]), 200);
        setTimeout(() => setTermLines(p => [...p, { text: '1    node -e require...' }]), 350);
        setTimeout(() => setTermLines(p => [...p, { text: '12   sh' }]), 500);
        setTimeout(() => setTermLines(p => [...p, { text: '18   ps aux' }]), 650);
        setTimeout(() => setView('procs'), 200);
      }, 2200),

      setTimeout(() => {
        setStep(4);
        setTermLines(p => [...p, { text: '/ # exit' }]);
        setTimeout(() => setTermLines(p => [...p, { text: 'docker logs app', isCmd: true }]), 200);
        setTimeout(() => setTermLines(p => [...p, { text: 'Server started on port 3000' }]), 400);
        setTimeout(() => setTermLines(p => [...p, { text: 'GET / 200 12ms' }]), 550);
        setTimeout(() => setTermLines(p => [...p, { text: 'GET /favicon.ico 404 2ms' }]), 700);
        setTimeout(() => { setView('logs'); setBadge('LOGS'); }, 300);
      }, 3000),

      setTimeout(() => {
        setStep(5);
        setTermLines(p => [...p, { text: 'docker logs --tail 5 --timestamps app', isCmd: true }]);
        setTimeout(() => setTermLines(p => [...p, { text: '2024-01-15T10:23:41Z Server started on port 3000' }]), 250);
        setTimeout(() => setTermLines(p => [...p, { text: '2024-01-15T10:23:45Z GET / 200 12ms' }]), 400);
        setTimeout(() => setView('logs-ts'), 200);
      }, 3700),
...
const AnimSystemCmds = ({ onDone }: { onDone: () => void }) => {
  const [termLines, setTermLines] = useState<{ text: string; isCmd?: boolean; isSuccess?: boolean }[]>([]);
  ...
  useEffect(() => {
    const timers = [
      setTimeout(() => {
        ...
        setTimeout(() => setTermLines(p => [...p, { text: 'TYPE            SIZE      RECLAIMABLE' }]), 200);
        setTimeout(() => setTermLines(p => [...p, { text: 'Images          452MB     234MB (52%)' }]), 350);
        setTimeout(() => setTermLines(p => [...p, { text: 'Containers      12MB      12MB (100%)' }]), 500);
        setTimeout(() => setTermLines(p => [...p, { text: 'Local Volumes   88MB      0B (0%)' }]), 650);
        setTimeout(() => setTermLines(p => [...p, { text: 'Build Cache     156MB     156MB (100%)' }]), 800);
        setTimeout(() => setMetrics({ images: 452, containers: 12, volumes: 88, cache: 156 }), 300);
      }, 200),

      setTimeout(() => {
        setStep(2);
        setTermLines(p => [...p, { text: 'docker system info', isCmd: true }]);
        setTimeout(() => setTermLines(p => [...p, { text: 'Containers: 3  Running: 1  Stopped: 2' }]), 200);
        setTimeout(() => setTermLines(p => [...p, { text: 'Images: 8' }]), 350);
        setTimeout(() => setTermLines(p => [...p, { text: 'Server Version: 24.0.5' }]), 500);
        setTimeout(() => setTermLines(p => [...p, { text: 'Storage Driver: overlay2' }]), 650);
        setTimeout(() => setTermLines(p => [...p, { text: 'Kernel: 5.15.0-91  |  OS: Ubuntu 22.04.3 LTS' }]), 800);
      }, 1500),

      setTimeout(() => {
        setStep(3);
        setTermLines(p => [...p, { text: 'docker container prune', isCmd: true }]);
        setTimeout(() => setTermLines(p => [...p, { text: 'WARNING: This will remove all stopped containers.' }]), 200);
        setTimeout(() => setTermLines(p => [...p, { text: 'Deleted Containers: a1b2c3, e5f6g7' }]), 400);
        setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => setTermLines(p => [...p, { text: 'Deleted Images: sha256:a1..., sha256:b2...' }]), 250);
        setTimeout(() => {
...
const AnimCheatSheet = ({ onDone }: { onDone: () => void }) => {
  const [termLines, setTermLines] = useState<{ text: string; isCmd?: boolean; isSuccess?: boolean }[]>([]);
    ...
    CHEAT_CMDS.forEach((entry, i) => {
      ...
      timers.push(setTimeout(() => {
        ...
        entry.output.forEach((line, j) => {
          timers.push(setTimeout(() => setTermLines(p => [...p, { text: line, isSuccess: j === entry.output.length - 1 }]), (j + 1) * 120));
        });
      }, base));
    });
    timers.push(setTimeout(onDone, CHEAT_CMDS.length * 600 + 400));
    return () => timers.forEach(clearTimeout);
  }, [onDone]);
...
const Level6Interactive = () => {
  ...
  const handleAnimDone = useCallback(() => {
    ...
    if (next.size === 5 && !levelDone) {
      setLevelDone(true);
      if (!completedLevels.includes(6)) completeLevel(6);
    }
              ...
              {active && (
                <motion.div key={animKey} className="absolute inset-0" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                  ...
                  <div className="absolute inset-0 pt-8">
                    {active === 'image-cmds' && <AnimImageCmds onDone={handleAnimDone} />}
                    {active === 'container-cmds' && <AnimContainerCmds onDone={handleAnimDone} />}
                    {active === 'exec-logs' && <AnimExecLogs onDone={handleAnimDone} />}
                    {active === 'system-cmds' && <AnimSystemCmds onDone={handleAnimDone} />}
                    {active === 'cheat-sheet' && <AnimCheatSheet onDone={handleAnimDone} />}
                  </div>
                </motion.div>
              )}
            </AnimatePresence>
